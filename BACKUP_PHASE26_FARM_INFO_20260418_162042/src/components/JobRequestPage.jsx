...
import { ArrowLeft, Sparkles, Camera, Loader2, CheckCircle, Zap, MapPin, Navigation } from 'lucide-react';
...
export default function JobRequestPage({ onBack, onSuccess, prefillJob }) {
  ...
  async function handleSubmit() {
    ...
    // PHASE 25: GPS 좌표 필수 — null이면 등록 차단
    if (gpsLat === null || gpsLng === null) {
      setError('📍 위치를 확인해주세요. 아래 "내 위치 사용" 버튼을 눌러주세요.');
      return;
    }
    ...
    try {
      await createJob({
        ...
        // PHASE 25: fallback 제거 — GPS 실좌표만 전송
        lat: gpsLat,
        lng: gpsLng,
        date,
      ...
      if (prefillJob) {
        try {
          trackClientEvent('job_copy_submitted', { jobId: prefillJob.id, prefilled: true });
        } catch (_) {}
      }
      setDone(true);
