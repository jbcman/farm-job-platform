...
import {
  ...
  Banknote, Clock, ChevronDown, Sprout,
} from 'lucide-react';
...
function BottomCard({ job, onQuickApply, onFallback, onDismiss, onViewMyApplications }) {
...
export default function JobMapView({ onBack, onViewDetail, onViewMyApplications }) {
  ...
  const leafletMarkers   = useRef([]);   // { id, marker } 쌍으로 저장
  ...
  useEffect(() => {
    ...
    safeJobs.forEach(job => {
      ...
      const marker = L.marker([job.lat, job.lng], { icon: makeJobIcon(job, isSelected) })
        .addTo(map)
        .on('click', (e) => {
          ...
          if (selectedIdRef.current && selectedIdRef.current !== job.id) {
            ...
            if (prev) {
              const prevJob = safeJobs.find(j => j.id === selectedIdRef.current);
              if (prevJob) prev.marker.setIcon(makeJobIcon(prevJob, false));
            }
          }

          // 현재 마커 선택 상태 아이콘
          marker.setIcon(makeJobIcon(job, true));
          selectedIdRef.current = job.id;

          setSelectedJob(job);
          try { map.flyTo([job.lat, job.lng], Math.max(map.getZoom(), MY_ZOOM), { duration: 0.45 }); } catch (_) {}
      ...
      leafletMarkers.current.push({ id: job.id, marker });
    });
  ...
  useEffect(() => {
    if (!selectedJob && selectedIdRef.current) {
      const prev = leafletMarkers.current.find(m => m.id === selectedIdRef.current);
      if (prev) {
        const prevJob = markers.find(j => j.id === selectedIdRef.current);
        if (prevJob) prev.marker.setIcon(makeJobIcon(prevJob, false));
      }
