...
function reengageUnselectedApplicants(newJob) {
    try {
        ...
        // Phase 12: 재매칭 성과 추적 (최상위 1회 로깅)
        try {
            trackEvent('reengage_match_found', {
                jobId:  newJob.id,
                userId: newJob.requesterId,
                meta: {
                    matchedCount: unselected.length,
                    category:     newJob.category,
                },
            });
            console.log(`[REENGAGE_MATCH_FOUND] jobId=${newJob.id} matchedCount=${unselected.length}`);
        } catch (e) {
            console.error('[REENGAGE_MATCH_FOUND_ERROR]', e.message);
        }
