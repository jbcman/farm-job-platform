 ...
 */
/**
 * STEP 4: jobId를 포함한 딥링크 생성
 * ${SERVICE_DOMAIN}/jobs/${jobId}?source=kakao
 */
function buildDeepLink(jobId) {
    return `${SERVICE_DOMAIN}/jobs/${jobId}?source=kakao`;
}
...
async function sendApplyAlert({ job, worker, farmer } = {}) {
    ...
    const link     = job.id ? buildDeepLink(job.id) : SERVICE_DOMAIN;
    const message  =
`📢 새로운 지원 도착!
...
module.exports = { sendJobAlert, sendApplyAlert, sendKakaoReal, sendKakaoMock, buildJobMatchTemplate, sentCache };
