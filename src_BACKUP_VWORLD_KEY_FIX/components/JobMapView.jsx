...
// ── 상수 ────────────────────────────────────────────────────────
const DEFAULT_CENTER = MAP_CONFIG.DEFAULT_CENTER; // [36.5, 127.5] — 한국 중심
const DEFAULT_ZOOM   = MAP_CONFIG.DEFAULT_ZOOM;
const MY_ZOOM        = MAP_CONFIG.MY_ZOOM;
...
function MapController({ flyTo, onMapClick }) {
  ...
  // 빈 화면 방지: 마운트 후 invalidateSize (컨테이너 크기 변경 시 타일 재계산)
  useEffect(() => {
    if (!map) return;
    const timers = [
  ...
  useEffect(() => {
    if (!map) return;
    if (flyTo && Array.isArray(flyTo) && isValidLatLng(flyTo[0], flyTo[1])) {
      if (JSON.stringify(flyTo) !== JSON.stringify(prev.current)) {
        prev.current = flyTo;
...
export default function JobMapView({ onBack, onViewDetail }) {
  ...
  function handleMarkerClick(job) {
    setSelectedJob(job);
    if (isValidLatLng(job.lat, job.lng)) {
      setFlyTo([job.lat, job.lng]);
    }
    try { trackClientEvent('map_marker_click', { jobId: job.id }); } catch (_) {}
  ...
  // 좌표 안전화: myPos가 유효한 [lat, lng]일 때만 사용, 아니면 fallback
  const validMyPos = Array.isArray(myPos) && isValidLatLng(myPos[0], myPos[1]);
  const mapCenter  = validMyPos ? myPos : DEFAULT_CENTER;
  const mapZoom    = validMyPos ? MY_ZOOM : DEFAULT_ZOOM;
  ...
  return (
    <div className="relative" style={{ height: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ── 마커 수 배지 ───────────────────────────────────────── */}
      {!isLoading && safeMarkers.length > 0 && (
        <div className="absolute top-16 right-4 z-[999] mt-2">
          <div className="bg-farm-green text-white text-xs font-bold px-3 py-1.5 rounded-full shadow">
            일자리 {safeMarkers.length}건
          </div>
        </div>
      )}

      {/* ── 지도 (VWorld WMTS) ──────────────────────────────────── */}
      <div style={{ flex: 1, width: '100%', minHeight: 420, position: 'relative' }}>
        <MapContainer
          center={mapCenter}
          zoom={mapZoom}
      ...
      {/* ── 빈 상태 안내 ─────────────────────────────────────── */}
      {!isLoading && safeMarkers.length === 0 && !error && (
