...
router.get('/my', async (req, res) => {
  ...
  let user = await db.prepare('SELECT id, referralCode, referralCount FROM users WHERE id = ?').get(userId);
  ...
  if (!user.referralCode) {
    ...
    await db.prepare('UPDATE users SET referralCode = ? WHERE id = ?').run(code, userId);
...
router.post('/use', async (req, res) => {
  ...
  const me = await db.prepare('SELECT id, referredBy FROM users WHERE id = ?').get(userId);
  ...
  const referrer = await db.prepare('SELECT id FROM users WHERE referralCode = ?').get(code.toUpperCase());
  ...
  await db.prepare('UPDATE users SET referredBy = ? WHERE id = ?').run(referrer.id, userId);
...
router.post('/reward', async (req, res) => {
  ...
  try {
    const user = await db.prepare('SELECT referredBy FROM users WHERE id = ?').get(userId);
    if (!user?.referredBy) return res.json({ ok: true, rewarded: false });

    // 이미 보상 받은 경우 (첫 완료 외 중복 방지) → referralRewarded 플래그 체크
    const alreadyRewarded = await db.prepare('SELECT referralRewarded FROM users WHERE id = ?').get(userId);
    if (alreadyRewarded?.referralRewarded) return res.json({ ok: true, rewarded: false });

    await db.prepare('UPDATE users SET referralCount = COALESCE(referralCount, 0) + 1 WHERE id = ?').run(user.referredBy);
    await db.prepare('UPDATE users SET referralRewarded = 1 WHERE id = ?').run(userId);
