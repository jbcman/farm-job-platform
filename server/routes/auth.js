 ...
 * C = 효과 후 결제 (관리자 후속 수금)
 */
function assignABGroup(userId) {
    const r = Math.random();
    const group = r < 0.334 ? 'A' : r < 0.667 ? 'B' : 'C';
    db.prepare('UPDATE users SET abGroup = ? WHERE id = ?').run(group, userId);
    return group;
}
...
router.post('/login', (req, res) => {
    ...
    if (!user) {
        ...
        try {
            ...
            db.prepare('UPDATE users SET referralCode = ? WHERE id = ?').run(code, id);
            user = { ...user, referralCode: code };
        } catch (_) {}
        // PHASE SCALE+: A/B 그룹 자동 배정 (신규 가입 시)
        try {
            const abGroup = assignABGroup(id);
            user = { ...user, abGroup };
